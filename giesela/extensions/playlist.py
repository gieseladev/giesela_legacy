...
class PlaylistCog(commands.Cog, name="Playlist"):
    ...
    async def playlist(self, ctx: Context, *, playlist: UnquotedStr = None):
        ...
        if playlist:
            ...
            await self.bot.help_command.command_callback(ctx, command=self.playlist.qualified_name)
            return
    ...
    async def playlist_show(self, ctx: Context):
        ...
        paginator = EmbedPaginator(template=template, fields_per_embed=5)
