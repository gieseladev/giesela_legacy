...
import inspect
import logging
from typing import Callable, Dict, List, Set, TYPE_CHECKING, Union

from discord import Client, Embed, Emoji
from discord.ext.commands import Cog, CogMeta, Command, Context
...
class CogGroupMeta(CogMeta):
    """Metaclass for `CogGroup`.

    This is like `CogMeta` but loads the commands and listeners from its predecessors.
    """

    def __new__(mcs, *args, **kwargs) -> "CogGroupMeta":
        cls: CogMeta = super().__new__(mcs, *args, **kwargs)

        commands: Set[Command] = set(getattr(cls, "__cog_commands__"))
        listeners: Set[Callable] = set(getattr(cls, "__cog_listeners__"))

        for base in cls.__mro__:
            if isinstance(base, CogMeta):
                commands.update(getattr(base, "__cog_commands__"))
                listeners.update(getattr(base, "__cog_listeners__"))

        cls.__cog_commands__ = list(commands)
        cls.__cog_listeners__ = tuple(listeners)

        return cls


class CogGroup(Cog, metaclass=CogGroupMeta):
    """Cog mechanism which loads inherited commands and listeners."""
    ...
