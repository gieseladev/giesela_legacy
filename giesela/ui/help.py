...
from giesela.lib import GIESELA_HELP_FORMATTER
from . import text
from .abstract import HasBot, Stoppable
from .interactive import AbstractCogGroupMeta, InteractableEmbed, MessageableEmbed, emoji_handler
...
async def get_command_help(ctx: Context, *cmds: str) -> Optional[Embed]:
    ...
    embeds = await GIESELA_HELP_FORMATTER.get_help_for(cmd, prefix=None, command_filter=None, help_command_name=ctx.invoked_with)
    return embeds[0]
...
class AutoHelpEmbed(HasBot, HasHelp, Cog, metaclass=AbstractCogGroupMeta):
    channel: TextChannel
