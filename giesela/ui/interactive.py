...
from .ui_utils import CogGroup, CogGroupMeta, EmbedLimits, EmojiType, MenuCommandGroup, format_embed
...
class AbstractCogGroupMeta(abc.ABCMeta, CogGroupMeta):
    """Metaclass required to make `CogGroup` work with `abc.ABC`"""
    ...


class MessageableEmbed(HasListener, HasBot, HasUser, EditableEmbed, MessageHandler, Startable, Stoppable, CogGroup, metaclass=AbstractCogGroupMeta):
    user: Optional[User]
    ...
    def __init__(self, channel: TextChannel, *,
        ...
        self._group.add_listener(self.on_command_error)
        self._group.add_listener(self.on_error)
        self._group.add_cog(self)
    ...
    async def on_error(self, event: str, *args, **kwargs):
        log.exception(f"{self} Error in {event} ({args}, {kwargs})")
    ...
    async def on_command_error(self, ctx: Optional[Context], exception: Exception):
        ...
        log.exception("CommandError:", exc_info=exception, extra=dict(tags=tags))
